...
class PlayScene extends BaseScene {
 ...
 // * Subtracts time from countdown text
 countDown() {
  this.initialTime--;
 ...
 createBird() {
  // Adds "Bird" (Goku), sets startPosition, scale, etc.
 ...
 // *! GAMEOVER & Restart
 checkGameStatus() {
  // function starts when bird is out of bounds
  // LATER: collide with pipes
  if (this.bird.getBounds().bottom >= this.config.height || this.bird.y <= 0) {
   this.gameOver();
  }
  // will update pipe positon
 }
 ...
 placePipe(uPipe, lPipe) {
  ...
  //Phaser.Math Calculate the distance between two sets of coordinates (points).
  // (x1, y1, x2, y2)
 ...
 recyclePipes() {
  // with out this function the pipes will go on forever
  // empty array
  const tempPipes = [];
  // for each pipe that is out of bounds
  this.pipes.getChildren().forEach((pipe) => {
   if (pipe.getBounds().right <= 0) {
    // recycle pipe
    // push sprites in array
    tempPipes.push(pipe);
